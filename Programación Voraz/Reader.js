...
function Reader() {
  try {
		const filePath = "../entradas/entrada.txt";
		...
		for (let i = 1; i <= numero_materias; i++) {
			...
			materiasJSON.materias.push({
				nombre: materia[0],
				cupos: parseInt(materia[1]),
			});
		}

		const numero_estudiantes = parseInt(
			contenido_l[numero_materias + 1].trim()
		);
		let index = 1;
		...
		while (true) {
			...
			for (let i = posicion_a + 1; i <= numero_materias + posicion_a; i++) {
				...
				materias_estudiantes.push({
					nombre_materia: materia_estudiante[0],
					prioridad: parseInt(materia_estudiante[1]),
				});
			}
			estudiantesJSON.estudiantes.push({
				nombre: estudiante[0],
				numero_materias: numero_materias,
