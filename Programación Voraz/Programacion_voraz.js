...
function quicksort(array, left, right, boolean, boolean2, materiasJSON) {
    if (left < right) {
        let pivotIndex = partition(array, left, right, boolean, boolean2, materiasJSON);
    ...
    for (let j = left; j < right; j++) {
        if (
            boolean
                ...
                : (boolean2 ? array[j].insatisfaccion >= pivot.insatisfaccion : array[j].numero_materias <= pivot.numero_materias)
        ) {
            ...
            if (!boolean) {
                ...
                if (boolean2) {
                    if (array[j].insatisfaccion > pivot.insatisfaccion) {
                        ...
                        if (array[j].materias_estudiante.length < pivot.materias_estudiante.length) {
                            i++;
                            let temp = array[i];
                            array[i] = array[j];
                            array[j] = temp;
                    ...
                    if (array[j].numero_materias < pivot.numero_materias) {
                        ...
                        if (array[j].materias_estudiante[0].prioridad > pivot.materias_estudiante[0].prioridad) {
                            i++;
                            let temp = array[i];
                            array[i] = array[j];
                            array[j] = temp;
                        } else if (
                            array[j].materias_estudiante[0].prioridad == pivot.materias_estudiante[0].prioridad) {
                            let cupos_array = 0;
                            let cupos_pivot = 0;
                            for (let materia = 0; materia < materiasJSON.numero_materias; materia++) {
                                for (let x = 0; x < array[j].numero_materias; x++) {
                                    if (
                                        array[j].materias_estudiante[x].nombre_materia == materiasJSON.materias[materia].nombre) {
                                        cupos_array += materiasJSON.materias[materia].cupos;
                                    }
                                    if (
                                        pivot.materias_estudiante[x].nombre_materia == materiasJSON.materias[materia].nombre) {
...
function matricularMaterias(materiasJSON, estudiantes, estudiantesMatriculados) {
    ...
    while (contador != materiasJSON.numero_materias) {
        ...
        if (estudiantes[i].materias_estudiante[0]) {
            ...
            for (x = 0; x < materiasJSON.materias.length; x++) {
                ...
                if (estudiantes[i].materias_estudiante[0]) {
                    ...
                    if (estudiantes[i].materias_estudiante[0].nombre_materia == materiasJSON.materias[x].nombre) {
                        ...
                        if (materiasJSON.materias[x].cupos > 0) {
                            ...
                            let estudianteExistenteIndex =
                                estudiantesMatriculados.findIndex((estudiante) => estudiante.nombre === estudiantes[i].nombre);
                            ...
                            if (estudianteExistenteIndex !== -1) {
                                estudiantesMatriculados[estudianteExistenteIndex].materias_asignadas.push(estudiantes[i].materias_estudiante[0]);
                            } else {
                            ...
                            if (cantidadAnteriorMaterias != estudiantes[i].materias_estudiante.length) {
                                x = 0
                                cantidadAnteriorMaterias = estudiantes[i].materias_estudiante.length
                            }
                        }
                    }
                }
        ...
        if (i == estudiantes.length - 1) {
            ...
            for (let materia_index = 0; materia_index < materiasJSON.materias.length; materia_index++) {
...
function calcularInsatisfaccionParcial(estudiantes, estudiantesMatriculados, i) {
    function γ(x) {
        return 3 * x - 1;
    }
    ...
    for (let j = 0; j < estudiantesMatriculados.length; j++) {
        if (
            estudiantes[i].nombre == estudiantesMatriculados[j].nombre
        ) {
            let suma = estudiantes[i].prioridadTotal;
            for (let materia = 0; materia < estudiantesMatriculados[j].materias_asignadas.length; materia++) {
                suma -= estudiantesMatriculados[j].materias_asignadas[materia].prioridad;
            ...
            estudiantes[i] = {
                ...estudiantes[i],
                numero_materias_asignadas: estudiantesMatriculados[j].materias_asignadas.length,
            };
        }
    }

    //genera la insatisfaccion
    const insatisfaccion = (1 - estudiantes[i].numero_materias_asignadas / estudiantes[i].numero_materias) *
...
function calcularInsatisfaccionTotal(estudiantes) {
    let resultado = 0
    for (let i = 0; i < estudiantes.length; i++) {
        console.log('nombre', estudiantes[i].nombre, 'insatisfaccion:', estudiantes[i].insatisfaccion)
